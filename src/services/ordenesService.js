  ...
  try {
    ...
    if (carrito) {
      ...
      for (let pto of productosCarrito) {
        ...
        ptoStock["stock"]--;
        if (ptoStock["stock"] == 0) {
          return { estado: "stockFalse", producto: ptoStock };
        }
      ...
      const sms = await twilioConfig.messages.create({
        ...
        from: "+19379362022",
        to: "+56933982608", //Por ahora este numero para que funcione en twilio. Dsps deberia ser el telefono del user
      });*/
    }catch (error) {
      loggerError.error(error);
